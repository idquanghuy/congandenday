...
exports.checkSession = async (phone, refresh = false) => {
    ...
    return ((new Date() - dataPhone.loginAt) > 1740000 || refresh) ? await this.refreshToken(phone) : ({
        success: true,
...
exports.getHistory = async (phone, configHistory, hours = 24) => {
    try {
        ...
        if (configHistory.dataType == 'noti') {
            ...
            let dataHistory = [], i = 0;
            console.log(response.message.data.notifications)
...
exports.getBalance = async (phone) => {
    ...
    let options = {
        method: 'POST',
        url: 'https://owa.momo.vn/api/SOF_LIST_MANAGER_MSG',
        headers: {
            'requestkey': requestkey,
            'userid': phone,
    ...
    return new Promise((resolve, reject) => {
        request(options, async (err, response, body) => {
            try {
                ...
                await momoModel.findOneAndUpdate({ phone }, { $set: { description: 'SOF_LIST_MANAGER_MSG| Có lỗi xảy ra ' + err.message || err } })
                return resolve({
                    success: false,
